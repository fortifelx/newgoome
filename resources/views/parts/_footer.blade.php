<footer>
    <div class="row">
        <div class="col-md-3 col-md-offset-1 col-sm-4 col-xs-12">
            <ul class="footer_list">
                <li>
                    <a href="{{route('help')}}">О нас</a>
                </li>
                <li>
                    <a href="{{route('help')}}">Помощь покупателям</a>
                </li>
                <li>
                    <a href="{{route('rules')}}">Как продавать</a>
                </li>
                <li>
                    <a href="{{route('help')}}">Участие в офлайн-маркете</a>
                </li>
                <li>
                    <a href="{{route('help')}}">Требования к магазинам</a>
                </li>
                <li>
                    <a href="{{route('rules')}}">Правила сайта</a>
                </li>
                <li>
                    <a href="{{route('rules')}}">Политика конфиденциальности</a>
                </li>
            </ul>
        </div>
        <div class="col-md-2 col-md-offset-1 col-sm-2 col-sm-offset-1 col-xs-12 fixed">
            <ul class="footer_list">
                <li>
                    <a href="{{route('woman')}}">Женщинам</a>
                </li>
                <li>
                    <a href="{{route('man')}}">Мужчинам</a>
                </li>
                <li>
                    <a href="{{route('children')}}">Детям</a>
                </li>
                <li>
                    <a href="{{route('accessories')}}">Аксессуары</a>
                </li>
                <li>
                    <a href="{{route('house')}}">Дом и быт</a>
                </li>
                <li>
                    <a href="{{route('food')}}">Еда и напитки</a>
                </li>
                <li>
                    <a href="{{route('contacts')}}">Контакты</a>
                </li>
            </ul>
        </div>
        <div class="col-md-4 col-md-offset-1 col-sm-4 col-sm-offset-1 col-xs-12">
            <p class="social_text">Следите за нами в социальных сетях</p>
            <div class="social">
                <a target="_blank" href=""><img src="{{ asset('assets/img/network/facebook.svg') }}" alt=""></a>
                <a target="_blank" href=""><img src="{{ asset('assets/img/network/instagram.svg') }}" alt=""></a>
                <a target="_blank" href=""><img src="{{ asset('assets/img/network/telegram.svg') }}" alt=""></a>
                <a target="_blank" href=""><img src="{{ asset('assets/img/network/what.svg') }}" alt=""></a>
                <a target="_blank" href=""><img src="{{ asset('assets/img/network/vk.svg') }}" alt=""></a>
            </div>
        </div>
    </div>
    <div class="copy">
        <div class="col-md-2 col-md-offset-5 col-sm-3 col-sm-offset-0 col-xs-12 fix">
            &copy;2018 OnlineStore
        </div>
        <div class="col-md-4 col-md-offset-1 col-sm-5 col-sm-offset-2 col-xs-12 fix">
            Design&amp;Development by <a target="_blank" href="http://it-is.agency/">IS.Agency</a>
        </div>
    </div>
</footer>